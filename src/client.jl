...
convert_response(::Type{Float64}, response) = typeof(response) == String ? parse(Float64,response)::Float64 : float(response)
...
import Base: ==
==(A::Nullable{T}, B::Nullable{U}) where  {T<:AbstractString, U<:AbstractString} = A.value == B.value
==(A::Nullable{T}, B::Nullable{U}) where {T<:Number, U<:Number} = A.value == B.value
...
function convert_response(::Type{Nullable{T}}, response) where {T<:Number}
    if response == nothing
       Nullable{T}(nothing)
   elseif typeof(response) <: T
        Nullable{T}(response)
    else
       response
    end
...
function convert_response(::Type{Nullable{T}}, response) where {T<:AbstractString}
    if response == nothing
       Nullable{T}(nothing)
    else
       Nullable{T}(response)
...
# redundant
function convert_response(::Type{Array{Nullable{T}, 1}}, response) where {T<:Number}
    if response == nothing
        Array{Nullable{T}, 1}(nothing)
   else
...
function convert_response(::Type{Array{Nullable{T}, 1}}, response) where {T<:AbstractString}
    if response == nothing
        Array{Nullable{T}, 1}(nothing)
   else
        r = Array{Nullable{T},1}()
