import Sockets.connect, Sockets.TCPSocket, Sockets.StatusActive, Sockets.StatusOpen
...
function is_connected(conn::RedisConnectionBase)
    conn.socket.status == StatusActive || conn.socket.status == StatusOpen|| conn.socket.status == Base.StatusPaused
end
